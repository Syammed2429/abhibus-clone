...
const DisplayBusDetails = () => {
    ...
    const getData = async (from, to) => {
        ...
        query["from"]=from;
        query["to"] = to
        const {data} = await axios.get(`http://localhost:2924/bus?from=${from}&to=${to}`)
        setBusData(data)
    ...
    const sorted_data=async(from, to,sorting)=>{
        const {data} = await axios.get(`http://localhost:2924/bus/sort?from=${from}&to=${to}&sorting=${sorting}`);
        setBusData(data);
                ...
                <div className="margin_display_d">
                    {busData.map((e) => (
                        <div key={e._id} className="flex_display_d border_display_d">
                            ...
                            <button className="button_display_d" onClick={()=>{
